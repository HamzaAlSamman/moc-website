...
export default function CitizenProfileCard({ citizen, locale = "ar" }) {
  ...
  const states = {
    UNSUBMITTED: { icon: IdCard, tone: "border-slate-200 bg-slate-50 text-slate-700", title: isAr ? "البريد موثق — الهوية لم تُرفع" : "Email verified — identity not submitted", body: isAr ? "ارفع صور الهوية لتتمكن من الحجز." : "Submit identity images to unlock booking." },
    PENDING: { icon: Clock3, tone: "border-amber-200 bg-amber-50 text-amber-900", title: isAr ? "الهوية قيد المراجعة" : "Identity under review", body: isAr ? "سنرسل القرار إلى بريدك عند اكتمال المراجعة." : "We will email you when review is complete." },
  ...
  const current = states[state] || states.UNSUBMITTED; const Icon = current.icon;
  return <div className="space-y-6"><header><p className="text-xs font-black uppercase tracking-[0.18em] text-[#A48E68]">{isAr ? "حساب المواطن" : "Citizen account"}</p><h2 className="mt-2 text-3xl font-black text-[#002723]">{citizen.fullName}</h2><p className="mt-1 text-sm text-slate-500">{citizen.email} · •••• {citizen.nationalIdLast4}</p></header><div className={`rounded-2xl border p-5 ${current.tone}`}><Icon aria-hidden="true" /><h3 className="mt-3 font-black">{current.title}</h3><p className="mt-2 text-sm leading-6">{current.body}</p></div><div className="grid gap-3 sm:grid-cols-2"><Link href={`/${locale}/account/identity`} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-[#003D33] px-4 text-sm font-black text-[#003D33] transition hover:bg-[#003D33] hover:text-white focus-visible:ring-2 focus-visible:ring-[#A48E68]">{isAr ? "إدارة الهوية" : "Manage identity"}</Link><Link href={state === "VERIFIED" ? `/${locale}/calendar` : `/${locale}/account/bookings`} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-[#A48E68] px-4 text-sm font-black text-[#002723] transition hover:bg-[#b8a47f] focus-visible:ring-2 focus-visible:ring-[#003D33]">{state === "VERIFIED" ? (isAr ? "استعراض الفعاليات" : "Browse events") : (isAr ? "عرض حجوزاتي" : "View bookings")}</Link></div></div>;
}
