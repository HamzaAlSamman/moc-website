...
import { createBooking } from "@/lib/event-booking-service";
...
export async function POST(request, { params }) {
  ...
  if (
    !rateLimit(`citizen-booking:ip:${ip}`, 120, 60 * 60 * 1000)
    || !rateLimit(`citizen-booking:citizen:${session.citizenId}`, 30, 60 * 60 * 1000)
  ) return json({ error: "محاولات كثيرة، يرجى المحاولة لاحقاً" }, 429);

  const { id } = await params;
  try {
    const booking = await createBooking({ eventId: id, citizenId: session.citizenId });
    return json({ booking: citizenBookingPayload(booking) }, booking.idempotent ? 200 : 201);
  } catch (error) {
