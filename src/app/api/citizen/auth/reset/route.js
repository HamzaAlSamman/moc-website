import { resetCitizenPassword } from "@/lib/citizen-auth";
...
export async function POST(request) {
  ...
  if (!rateLimit(`citizen-reset:ip:${ip}`, 50, 60 * 60 * 1000)) {
    return citizenJson({ error: "محاولات كثيرة، يرجى المحاولة لاحقاً" }, 429);
  }
  try {
    return citizenJson(await resetCitizenPassword(await readCitizenAuthJson(request)));
  } catch (error) {
