...
import { canTransitionCopyright, validateUploadedDocument } from "@/lib/business-rules.mjs";
...
export async function PATCH(request, { params }) {
  ...
  if (session.role === "CULTURAL_CENTER_OFFICER" && applicationStatus === "pending_certificate") {
  ...
  // Certificate issuance: the studies officer must upload the actual signed
  // certificate — it is never generated by the system. Same document
  // validation as the citizen-facing uploads (PDF/JPEG/PNG/WEBP, signature
  // checked, size-capped).
  if (applicationStatus === "completed" && existing.applicationStatus === "pending_certificate") {
    try {
      validateUploadedDocument(certificateFile, "certificateFile");
    } catch (error) {
  ...
  if (applicationStatus === "pending_certificate" && existing.applicationStatus === "pending_center_delivery") {
    updateData.centerConfirmedAt = new Date();
    updateData.centerConfirmedById = session.userId;
  }
  if (applicationStatus === "completed" && existing.applicationStatus === "pending_certificate") {
    updateData.certificateFile = certificateFile;
    updateData.certificateIssuedAt = new Date();
    updateData.certificateIssuedById = session.userId;
  }
  ...
  } else if (applicationStatus === "pending_certificate") {
    // Center confirmed the deposit arrived — either the assessor or the head
    // who already worked this case can now upload and send the certificate.
    await notifyByRole(["STUDIES_ASSESSOR", "STUDIES_HEAD"], {
      type: "COPYRIGHT_CERTIFICATE_PENDING",
      titleAr: `جاهزة لرفع الشهادة وإرسالها: ${title}`,
      titleEn: `Ready for certificate upload: ${title}`,
      link,
    });
  }
