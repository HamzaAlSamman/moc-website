...
const CONTACT_TYPES = [
  ...
  { value: "thanks",     ar: "شكر",   en: "Appreciation" },
];

const T = {
  ar: {
    metaTitle: "مديرية التعاون الدولي – وزارة الثقافة السورية",
    title: "التواصل مع مديرية التعاون الدولي",
    subtitle:
      "قناة رسمية مخصّصة للجهات الحكومية والمنظمات والجمعيات الأهلية للتواصل مع مديرية التعاون الدولي في وزارة الثقافة وإرسال الشكاوى والاقتراحات.",
    audienceNote: "هذه الخدمة موجّهة للجهات الحكومية والمنظمات والجمعيات الأهلية.",
    responseNote: "المدة المتوقعة للرد تتراوح بين يومين وأسبوع عمل.",
    section1Title: "بيانات مقدم الطلب",
    section2Title: "تفاصيل الرسالة",
    nameLabel: "اسم مقدم الطلب",
    namePlaceholder: "الاسم الكامل لمقدم الطلب",
    workplaceLabel: "مكان عمل مقدم الطلب",
    workplacePlaceholder: "اسم الجهة أو المنظمة أو الجمعية",
    emailLabel: "البريد الإلكتروني",
    emailPlaceholder: "example@email.com",
    phoneLabel: "رقم الهاتف",
    phonePlaceholder: "+963xxxxxxxxx",
    contactTypeLabel: "موضوع التواصل",
    subjectLabel: "الموضوع",
    subjectPlaceholder: "عنوان مختصر لرسالتك",
    messageLabel: "نص الرسالة",
    messagePlaceholder: "يرجى كتابة نص الرسالة بشكل واضح...",
    imagesLabel: "المرفقات",
    imagesHint: "اختياري — يمكنك إرفاق صور تدعم رسالتك (بحد أقصى 5 صور، 5 ميجابايت لكل صورة).",
    ...
    successSubtitle: "تم استلام الرسالة بنجاح، وستقوم مديرية التعاون الدولي بمراجعتها والرد عليها بأسرع وقت ممكن.",
    nextStepsTitle: "ماذا بعد؟",
    nextStepsList: [
      "تتم مراجعة رسالتك من قبل مديرية التعاون الدولي في وزارة الثقافة.",
      "المدة المتوقعة للرد تتراوح بين يومين وأسبوع عمل.",
      "سيتم التواصل معكم عبر البريد الإلكتروني أو رقم الهاتف المسجل.",
    ],
    sendAnother: "إرسال رسالة أخرى",
    validationName: "يرجى إدخال اسم مقدم الطلب.",
    validationWorkplace: "يرجى إدخال مكان العمل.",
    validationEmail: "يرجى إدخال بريد إلكتروني صحيح.",
    validationPhone: "يرجى إدخال رقم الهاتف.",
    validationSubject: "يرجى إدخال موضوع الرسالة.",
    validationMessage: "يرجى كتابة نص الرسالة.",
  ...
  en: {
    metaTitle: "Directorate of International Cooperation – Syrian Ministry of Culture",
    title: "Contact the International Cooperation Directorate",
    subtitle:
      "An official channel for government entities, organizations, and civil associations to reach the Directorate of International Cooperation at the Ministry of Culture.",
    audienceNote: "This service is intended for government entities, organizations, and civil associations.",
    responseNote: "Expected response time is between two days and one working week.",
    section1Title: "Applicant Details",
    section2Title: "Message Details",
    nameLabel: "Applicant Name",
    namePlaceholder: "Full name of the applicant",
    workplaceLabel: "Applicant's Workplace",
    workplacePlaceholder: "Entity, organization, or association name",
    ...
    subjectPlaceholder: "A short title for your message",
    messageLabel: "Message",
    messagePlaceholder: "Write the details of your message clearly...",
    imagesLabel: "Attachments",
    imagesHint: "Optional — attach images that support your message (max 5 images, 5MB each).",
    ...
    successSubtitle: "Your message has reached the Directorate of International Cooperation and will be reviewed shortly.",
    nextStepsTitle: "What happens now?",
    nextStepsList: [
      "Your message is reviewed by the Directorate of International Cooperation.",
      "Expected response time is between two days and one working week.",
      "You will be contacted via the email or phone number you provided.",
    ],
    sendAnother: "Send Another Message",
    validationName: "Please enter the applicant name.",
    validationWorkplace: "Please enter the workplace.",
    validationEmail: "Please enter a valid email address.",
    validationPhone: "Please enter your phone number.",
    validationSubject: "Please enter the message subject.",
    validationMessage: "Please write the message body.",
...
export default function InternationalCooperationPage(props) {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.name.trim()) { setError(tForm.validationName); return; }
    if (!form.workplace.trim()) { setError(tForm.validationWorkplace); return; }
    if (!emailRegex.test(form.email.trim())) { setError(tForm.validationEmail); return; }
    if (!form.phone.trim() || form.phone === "+") { setError(tForm.validationPhone); return; }
    if (!form.subject.trim()) { setError(tForm.validationSubject); return; }
    if (!form.message.trim()) { setError(tForm.validationMessage); return; }

    setSubmitting(true);
    try {
      const res = await fetch("/api/cooperation-contact", {
        method: "POST",
            ...
            <form onSubmit={handleSubmit} className="space-y-8 text-start">
              ...
              <div className="bg-white rounded-3xl border border-[#b9a779]/20 shadow-sm overflow-hidden relative">
                ...
                <div className="px-8 sm:px-12 pb-8 sm:pb-12 pt-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="sm:col-span-2">
                    <Field label={tForm.nameLabel} required>
                      <input className={inputCls} value={form.name} onChange={(e) => set("name", e.target.value)} placeholder={tForm.namePlaceholder} />
                    </Field>
                  </div>
                  <div className="sm:col-span-2">
                    <Field label={tForm.workplaceLabel} required>
                      <input className={inputCls} value={form.workplace} onChange={(e) => set("workplace", e.target.value)} placeholder={tForm.workplacePlaceholder} />
                    </Field>
                  </div>
              ...
              <div className="bg-white rounded-3xl border border-[#b9a779]/20 shadow-sm overflow-hidden relative">
                ...
                <div className="px-8 sm:px-12 pb-8 sm:pb-12 pt-6 space-y-5">
                  <Field label={tForm.contactTypeLabel} required>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                      {CONTACT_TYPES.map((ct) => (
                        <label key={ct.value} className={`flex items-center justify-center gap-2.5 border rounded-xl px-4 py-3 cursor-pointer transition-all ${
                          form.contactType === ct.value
                            ? "border-[#054239] bg-[#054239]/8 text-[#054239]"
                            : "border-slate-200 hover:border-slate-300 text-slate-600"
                        }`}>
                          <input type="radio" name="contactType" value={ct.value} checked={form.contactType === ct.value} onChange={() => set("contactType", ct.value)} className="accent-[#054239] shrink-0" />
                          <span className="text-sm font-bold">{isRtl ? ct.ar : ct.en}</span>
                        </label>
                      ))}
                    </div>
                  </Field>
