...
// The certificate is uploaded by a studies officer (not generated by the
// system) — decode whatever document type was validated on the way in
// (validateUploadedDocument in business-rules.mjs) into a mail attachment.
function buildCertificateAttachment(submission) {
  const match = typeof submission.certificateFile === "string"
    && submission.certificateFile.match(/^data:(application\/pdf|image\/(?:jpeg|png|webp));base64,([A-Za-z0-9+/]+={0,2})$/);
  if (!match) return [];
  const ext = { "application/pdf": "pdf", "image/jpeg": "jpg", "image/png": "png", "image/webp": "webp" }[match[1]];
  return [{
    filename: `MOC-certificate-${submission.id}.${ext}`,
...
// ═══════════════════════════════════════════════════════════════════════════════
// EMAIL 4 — الشهادة الموقّعة (يُرسل عند completed، بعد رفعها من قِبل الدارس/رئيس
// القسم) + إيصال الرسم النهائي المُولَّد آلياً كالمعتاد.
// ═══════════════════════════════════════════════════════════════════════════════
export async function sendCompletedEmail(submission) {
  const fees = getFeesForMailer(submission.applicantRole);
  const receiptAttachments = await buildReceiptAttachment(submission, "final");
  const certificateAttachments = buildCertificateAttachment(submission);
  const attachments = [...certificateAttachments, ...receiptAttachments];
  ...
  await dispatchEmail(submission, {
    ...
    contentHtml: `
      <p style="font-size:14px;">
        ...
        للعمل <strong>«${esc(submission.workTitle)}»</strong>.
      </p>
      ...
      <div style="margin-top:20px;background:#f0fdf4;border:2px solid #86efac;border-radius:12px;padding:20px;text-align:center;">
        ...
        <h4 style="color:#15803d;margin:0 0 8px 0;font-size:16px;">شهادتك الرسمية مرفقة بهذه الرسالة</h4>
      </div>

      ${receiptNotice(receiptAttachments)}
      ...
      <div style="margin-top:18px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:13px;font-size:12px;color:#1e3a8a;line-height:1.6;">
        📌 احتفظ بهذه الرسالة والشهادة وإيصال الدفع المرفقين كوثائق رسمية. رقم المعاملة:
